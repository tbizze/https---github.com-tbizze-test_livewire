<div>
    ...
    {{-- Renderiza tabela --}}
    <x-mary-table :headers="$headers" :rows="$roles" striped @row-click="$wire.edit($event.detail.id)" with-pagination :sort-by="$sortBy" >
        @scope('actions', $roles)
            @can('admin.roles.delete')
            <x-mary-button icon="o-trash" wire:click="confirmDelete({{ $roles->id }})" spinner class="btn-sm btn-outline border-none text-error p-1" />
            @endcan
        @endscope
    </x-mary-table>
