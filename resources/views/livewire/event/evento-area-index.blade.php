<div>
    ...
    {{-- Renderiza tabela --}}
    <x-mary-table :headers="$headers" :rows="$evento_areas" striped @row-click="$wire.edit($event.detail.id)" with-pagination :sort-by="$sortBy" >
        @scope('actions', $evento_area)
            <x-mary-button icon="o-trash" wire:click="confirmDelete({{ $evento_area->id }})" spinner class="btn-sm btn-outline border-none text-error p-1" />
        @endscope
    </x-mary-table>
