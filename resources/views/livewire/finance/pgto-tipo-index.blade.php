<div>
    ...
    {{-- Renderiza tabela --}}
    <x-mary-table :headers="$headers" :rows="$pgto_tipos" striped @row-click="$wire.edit($event.detail.id)" with-pagination :sort-by="$sortBy">
        @scope('actions', $pgto_tipo)
            <x-mary-button icon="o-trash" wire:click="confirmDelete({{ $pgto_tipo->id }})" spinner class="btn-sm btn-outline border-none text-error p-1" />
        @endscope
    </x-mary-table>
