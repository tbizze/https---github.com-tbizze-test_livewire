<div>
    ...
    {{-- Renderiza tabela --}}
    <x-mary-table :headers="$headers" :rows="$movimento_grupos" striped @row-click="$wire.edit($event.detail.id)" with-pagination :sort-by="$sortBy" >
        @scope('actions', $movimento_grupo)
            <x-mary-button icon="o-trash" wire:click="confirmDelete({{ $movimento_grupo->id }})" spinner class="btn-sm btn-outline border-none text-error p-1" />
        @endscope
    </x-mary-table>
